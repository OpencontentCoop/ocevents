...
<div class="row">
  <div class="col-md-12">
        <span class="addtocalendar atc-style-blue">
            <a class="atcb-link">
              <i class="fa fa-calendar" aria-hidden="true"></i> {'Aggiungi al tuo calendario'|i18n('agenda')}
            </a>
            <var class="atc_event">
            <var class="atc_date_start">{$from_time|datetime( custom, '%Y-%m-%d %H:%i:%s')}</var>
            <var class="atc_date_end">{$to_time|datetime( custom, '%Y-%m-%d %H:%i:%s')}</var>
            <var class="recurring">{$recurrences.input.recurrencePattern}</var>
