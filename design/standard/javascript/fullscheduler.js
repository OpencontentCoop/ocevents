...
(function (factory) {
	...
	if (!$.fn.combobox || !$.fn.datepicker || !$.fn.radio || !$.fn.selectlist || !$.fn.spinbox) {
		throw new Error('Fullscheduler control requires combobox, datepicker, radio, selectlist, and spinbox.');
	}
	...
	var old = $.fn.fullscheduler;

	// FULLSCHEDULER CONSTRUCTOR AND PROTOTYPE

	var FullScheduler = function (element, options) {
		var self = this;

		this.$element = $(element);
		this.options = $.extend({}, $.fn.fullscheduler.defaults, options);

		// cache elements
		this.$startDate = this.$element.find('.start-datetime .start-date');
		this.$startTime = this.$element.find('.start-datetime .start-time');
		this.$endDate = this.$element.find('.end-datetime .end-date');
		this.$endTime = this.$element.find('.end-datetime .end-time');
		...
		this.$untilDate = this.$end.find('.end-on-date');
		...
		this.$endTime.combobox();
		// init start time
		if (this.$startTime.find('input').val() === '') {
			this.$startTime.combobox('selectByIndex', 0);
		}
		if (this.$endTime.find('input').val() === '') {
			this.$endTime.combobox('selectByIndex', 0);
		}
		...
		this.$untilDate.datepicker(this.options.untilDateOptions);
		...
		this.$element.find('.repeat-days-of-the-week .btn-group .btn').on('change.fu.fullscheduler', function (e, data) {
		...
		this.$element.find('.repeat-monthly .radio, .repeat-yearly .radio').on('change.fu.fullscheduler', $.proxy(this.changed, this));
	};

	FullScheduler.prototype = {
		constructor: FullScheduler,
		...
		changed: function (e, data, propagate) {
			...
			this.$element.trigger('changed.fu.fullscheduler', {
		...
		endSelectChanged: function (e, data) {
			...
			this.$untilDate.parent().addClass('hide');
			this.$untilDate.parent().attr('aria-hidden', 'true');

			if (val === 'after') {
				this.$endAfter.parent().removeClass('hide');
				this.$endAfter.parent().attr('aria-hidden', 'false');
			} else if (val === 'date') {
				this.$untilDate.parent().removeClass('hide');
				this.$untilDate.parent().attr('aria-hidden', 'false');
			}
		},
		...
		getValue: function () {
			...
			if (this.$endTime.combobox('selectedItem').value) {
				endTime = this.$endTime.combobox('selectedItem').value;
				endTime = endTime.toLowerCase();

			} else {
				endTime = this.$endTime.combobox('selectedItem').text.toLowerCase();
			}
			...
			var day, days, hasAm, hasPm, month, pos, startDateTime, endDateTime, type;

			startDateTime = '' + getFormattedDate(this.$startDate.datepicker('getDate'), '-');
			endDateTime = '' + getFormattedDate(this.$endDate.datepicker('getDate'), '-');
			...
			hasAm = (endTime.search('am') >= 0);
			hasPm = (endTime.search('pm') >= 0);
			endTime = $.trim(endTime.replace(/am/g, '').replace(/pm/g, '')).split(':');
			endTime[0] = parseInt(endTime[0], 10);
			endTime[1] = parseInt(endTime[1], 10);
			if (hasAm && endTime[0] > 11) {
				endTime[0] = 0;
			} else if (hasPm && endTime[0] < 12) {
				endTime[0] += 12;
			}
			...
			endDateTime += (endTime[0] < 10) ? '0' + endTime[0] : endTime[0];
			endDateTime += ':';
			endDateTime += (endTime[1] < 10) ? '0' + endTime[1] : endTime[1];
			endDateTime += (timeZone.offset === '+00:00') ? 'Z' : timeZone.offset;
			...
			if (repeat !== 'none') {
				if (end === 'after') {
					...
					duration = 'UNTIL=' + getFormattedDate(this.$untilDate.datepicker('getDate'), '') + ';';
				}
		...
		setValue: function (options) {
			var hours, i, item, l, minutes, period, recur, temp, startDate, startTime, endDate, endTime, timeOffset;
			...
			if (options.startDateTime) {
				...
				if (options.endDateTime) {
					temp = options.endDateTime.split('T');
					endDate = temp[0];
					...
					if (temp[1]) {
						...
						temp = hours + ':' + minutes + ' ' + period;
						this.$endTime.find('input').val(temp);
						this.$endTime.combobox('selectByText', temp);
					} else {
						endTime = '00:00';
					}
				}
			...
			if (options.recurrencePattern) {
				...
				if (recur.COUNT) {
					...
					startDate = temp;
					var utcUntilHours = this.setUtcTime(startDate, startTime, timezoneOffset);
					this.$untilDate.datepicker('setDate', utcUntilHours);
			...
			var utcEndHours = this.setUtcTime(endDate, endTime, timeOffset);
	...
	// FULLSCHEDULER PLUGIN DEFINITION

	$.fn.fullscheduler = function (option) {
		var args = Array.prototype.slice.call(arguments, 1);
		var methodReturn;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('fu.fullscheduler');
			var options = typeof option === 'object' && option;

			if (!data) {
				$this.data('fu.fullscheduler', (data = new FullScheduler(this, options)));
			}
	...
	$.fn.fullscheduler.defaults = {};

	$.fn.fullscheduler.Constructor = FullScheduler;

	$.fn.fullscheduler.noConflict = function () {
		$.fn.fullscheduler = old;
		return this;
	};
	...
	$(document).on('mousedown.fu.fullscheduler.data-api', '[data-initialize=fullscheduler]', function (e) {
		var $control = $(e.target).closest('.fullscheduler');
		if (!$control.data('fu.fullscheduler')) {
			$control.fullscheduler($control.data());
		}
	});

	// Must be domReady for AMD compatibility
	$(function () {
		$('[data-initialize=fullscheduler]').each(function () {
			var $this = $(this);
			if ($this.data('fullscheduler')) return;
			$this.fullscheduler($this.data());
		});
	});
